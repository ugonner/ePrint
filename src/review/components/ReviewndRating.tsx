import { useState } from "react";
import { useAsyncHelpersContext } from "../../shared/contexts/async-helpers";
import { getLocalUser } from "../../utils";
import { IAidServiceProfile } from "../../aid-service/interfaces/aid-service-profile";
import { getNumberRange } from "../../shared/components/form/DateSelector";
import { ReviewAndRatingDTO } from "../dtos/review-and-rating";
import { APIBaseURL, postData } from "../../shared/api/base";
import { IonCol, IonContent, IonGrid, IonIcon, IonItem, IonModal, IonRow, IonTextarea } from "@ionic/react";
import { starOutline, starSharp } from "ionicons/icons";
import { ServiceType } from "../enums/service";

export interface IReviewAndRatingProps {
  serviceType: ServiceType;
  serviceTypeEntityId: number;
  onCompletion?: () => void;
}

export const ReviewAndRate = ({
  serviceType,
  serviceTypeEntityId,
  onCompletion
}: IReviewAndRatingProps) => {
  const { setLoading, handleAsyncError } = useAsyncHelpersContext();
  
  const [reviewDto, setReviewDto] = useState<ReviewAndRatingDTO>({
    rating: 1,
    review: "",
    serviceType,
    serviceTypeEntityId
  } as ReviewAndRatingDTO);
  const [openReviewOverlay, setOpenReviewOverlay] = useState(false);

  const ratingNumbers = getNumberRange(1, 5);

  const rateService = async () => {
    try {
      setLoading({ isLoading: true, loadingMessage: "reviewing service" });
      await postData(`${APIBaseURL}/review`, {
        method: "post",
        ...reviewDto,
        serviceType,
        serviceTypeEntityId
      });
      setLoading({ isLoading: false, loadingMessage: "" });
      if(onCompletion) onCompletion();
    } catch (error) {
      handleAsyncError(error, "Error updating review");
    }
  };

  return (
    <>
    
    <IonGrid>
      <IonRow>
        <IonCol size="12">
          <h3>Rating and Review</h3>
          <p>Rate and then you can pass a review comment</p>
        </IonCol>
      </IonRow>
      <IonRow>
        <IonCol size="12">
          {ratingNumbers.map((rating) => (
            <span
              key={rating}
              role="button"
              className="ion-margin"
              onClick={() => {
                if(reviewDto.rating === rating) setReviewDto({} as ReviewAndRatingDTO);
                else {
                    setReviewDto({...reviewDto, rating});
                    setOpenReviewOverlay(true);
                }
              }}
              aria-label={rating === reviewDto.rating ? "reset rating" : `rate ${rating}`}
            >
                <IonIcon icon={reviewDto.rating >= rating ? starSharp : starOutline}></IonIcon>
            </span>
          ))}
        </IonCol>
      </IonRow>
    </IonGrid>
    <IonModal
    isOpen={openReviewOverlay}
    onDidDismiss={() => setOpenReviewOverlay(false)}
    >
        <IonContent>
            <IonItem>
                <IonTextarea
                label="Review Comment"
                labelPlacement="floating"
                value={reviewDto.review}
                onIonInput={(evt) => setReviewDto({...reviewDto, review: evt.detail.value as string})}
                />
            </IonItem>
            <IonItem>
                {
                    ["save", "cancel"].map((item) => (
                        <span key={item} className="ion-margin" role="button" onClick={() => {
                            if(item === "save") rateService();
                            setOpenReviewOverlay(false);
                            if(onCompletion) onCompletion();
                        }}>
                            {item}
                        </span>
                    ))
                }
            </IonItem>
        </IonContent>
    </IonModal>
    </>
  );
};
